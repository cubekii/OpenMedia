...
public:
    ...
    DynamicLoader(DynamicLoader&& other) noexcept : library_( other.library_ ) {
...
#  if defined( __unix__ ) || defined( __APPLE__ ) || defined( __QNX__ ) || defined( __Fuchsia__ )
        return (T)dlsym(library_, function);
#  elif defined( _WIN32 )
        return static_cast<T>(::GetProcAddress(library_, function));
#  else
#    error unsupported platform
#  endif
    }
